...
def say_hello_word_five_journalist (array,client)
    five_journalist = array.sample(5)
    five_journalist.each do |i|
        client.update("Hello #{i} ! #bonjour_monde @the_hacking_pro")
    end
end

def like_hello_word(client)
...
def follow_hello(client)
    ...
    client.search("#Hello_World", result_type: "recent").collect do |tweet|
        follow_array<< tweet.user.screen_name            
    end
    follow_array = follow_array.uniq
    
    follow_array.each do |client_follow|
        client.follow(client_follow)
    end


end

def master_twitter(client)
    client.filter(track:"#bonjour_monde") do |tweets|
     client.favorite(tweets) 
     puts tweets
    end
